#include "VMACubemapAllocation.h"

VMACubemapAllocation::VMACubemapAllocation(const VMAAllocator& allocator, const std::array<const void*, 6> data, const int width, const int height) :
	allocator(allocator)
{
	...
	allocator.CreateImage(image, allocation, width, height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, static_cast<uint32_t>(data.size()), VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT);
...
VMACubemapAllocation::~VMACubemapAllocation() {
